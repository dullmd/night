...
const config = {
  ...
  BOT_NAME: '𝑺𝑰𝑳𝑨 𝑴𝑰𝑵𝑰 𝑩𝑶𝑻',
  BOT_VERSION: '1.0.2',
  OWNER_NAME: '𝑺𝑰𝑳𝑨-𝑴𝑫',
  IMAGE_PATH: 'https://files.catbox.moe/36vahk.png',
  BOT_FOOTER: '> 𝒑𝒐𝒘𝒆𝒓 𝒃𝒚 𝒔𝒊𝒍𝒂-𝒎𝒅',
...
`.trim();
        ...
        case 'download': {
          ...
          try {
            let userCfg = {};
            try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
            const title = userCfg.botName || '© 𝒔𝒊𝒍𝒂𝒎𝒅  ᴍɪɴɪ';
...
`.trim();
        ...
        case 'tiktokdl': {
          try {
            ...
            let botName = cfg.botName || '𝒔𝒊𝒍𝒂-xᴅ';
...
`.trim();
        ...
        case 'settings': {
          ...
          try {
            let userCfg = {};
            try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
            const title = userCfg.botName || '©𝒔𝒊𝒍𝒂  𝒎ᴅ';
